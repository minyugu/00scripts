...
# determine user for preparing runas command /user parameter
if ( ($env:USERNAME) -eq "Administrator1" )
{
    $runasAccountName = "tier1admin"
} elseif ( ($env:USERNAME) -eq "SREreader" )
{
    $runasAccountName = "SREAssist"
} else {
    $notesWshell = New-Object -ComObject Wscript.Shell
    $notesWshell.Popup("The User can not run StrartTools",0,"Warning",0)
    Return
}
