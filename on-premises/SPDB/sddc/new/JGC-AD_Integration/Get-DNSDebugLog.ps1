...
function Get-DNSDebugLog
{
    ...
    Param(
        ...
        [ValidateSet('Client','DateTime','Protocol','Query','Query/Response','RecordType','SendReceive')]
    ...
    BEGIN
    {
      ...
      #
      # data sample from chinese data format, used for dnspattern below
      # 2024/12/2 15:51:54 166C PACKET  000001AB5710F890 UDP Rcv 10.191.24.4     c6b8   Q [0001   D   NOERROR] A      (14)esfregbiza2-sh(3)rel(4)spdb(3)com(0)
      #
       $dnspattern = "^(?<log_date>([0-9]{2,4}.[0-9]{1,2}.[0-9]{1,2}|[0-9]{1,2}.[0-9]{1,2}.[0-9]{2,4}|[0-9]{2,4}-[0-9]{2}-[0-9]{2})\s*[0-9: ]{7,8}\s*(PM|AM)?) ([0-9A-Z]{3,4} PACKET\s*[0-9A-Za-z]{8,16}) (?<protocol>UDP|TCP) (?<way>Snd|Rcv) (?<ip>[0-9.]{7,15}|[0-9a-f:]{3,50})\s*([0-9a-z]{4}) (?<QR>.) (?<OpCode>.) \[.*\] (?<QueryType>.*) (?<query>\(.*)"
       $returnselect =  #@{label="DateTime";expression={[datetime]::ParseExact($match.Groups['log_date'].value.trim(),"dd/MM/yyyy HH:mm:ss",$null)}},
                        @{label="DateTime";expression={Get-Date -Date ($match.Groups['log_date'].value)}},
    ...
    PROCESS
    {
        ...
        getDNSLogLines -DNSLog $DNSLog | % {
            ...
            if ($match.success )
            {
                Try
                {
                    if ( [String]::IsNullOrEmpty($RemoveDuplicateProperty) )
                    {
                    ...
                    {
                        $dnsRecord = $null
                        $dnsRecord = $true | Select-Object $returnselect
                        $duplicatePropertyFound = $dnsRecord | Process-HashTable -hashTable $HashTable -KeyProperty $RemoveDuplicateProperty -onlyKey
